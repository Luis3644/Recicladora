...
class _MapaGeneralOperadoresScreenState
  ...
  void _animCamera(LatLng dest, {double? zoom}) {
    ...
    final posAnim = _LatLngTween(begin: from, end: dest).animate(
      CurvedAnimation(parent: _cameraCtrl, curve: Curves.easeOutCubic),
    );
    final zoomAnim = Tween<double>(begin: zFrom, end: zTo).animate(
      CurvedAnimation(parent: _cameraCtrl, curve: Curves.easeOutCubic),
    );
  ...
  void _followUpdate(List<_OperadorActivo> ops) {
    ...
    final sel = ops.where((o) => o.id == _seleccionadoId).cast<_OperadorActivo?>()
        .firstWhere((_) => true, orElse: () => null);
    final target = sel?.posicion ?? ops.first.posicion;
    if (_ultimaPosSeguida != null && _samePt(_ultimaPosSeguida!, target)) return;
  ...
  String _firma(List<_OperadorActivo> ops) {
    ...
    return sorted.map((o) => '${o.id}:${o.posicion.latitude}:${o.posicion.longitude}').join('|');
  }
  ...
  LatLng? _parsePos(Map<String, dynamic> data) {
    final gp = data['ubicacion'] ?? data['ubicacion_actual'] ?? data['posicion'];
    ...
    double? parsear(dynamic v) {
      ...
      if (v is num) return v.toDouble(); // Ahora 'is num' funciona correctamente
      if (v is String) return double.tryParse(v.trim());
      return null;
    }

    // Usamos el nuevo nombre de la función aquí:
    final lat = parsear(data['latitud']   ?? data['latitude']  ?? data['lat'] ?? data['y']);
    final lng = parsear(data['longitud']  ?? data['longitude'] ?? data['lng'] ?? data['lon'] ?? data['x']);
      ...
      body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snap) {
          ...
          for (final doc in docs) {
            ...
            ops.add(_OperadorActivo(
              id: doc.id, nombre: nombre,
              posicion: pos, color: _colorOp(doc.id),
            ));
            tracked.add({
              'id'       : doc.id,
              'nombre'   : nombre,
              'camion'   : camion.isEmpty ? 'Sin camión asignado' : camion,
              'direccion': direccion.isEmpty ? 'Ubicación no disponible' : direccion,
            });
          }
          ...
          }).whereType<Polyline>().toList(growable: false);

          // Centro
          LatLng center = _fallbackCenter;
          if (ops.isNotEmpty) {
            center = LatLng(
              ops.map((o) => o.posicion.latitude ).reduce((a, b) => a + b) / ops.length,
              ops.map((o) => o.posicion.longitude).reduce((a, b) => a + b) / ops.length,
            );
          }
          ...
          return Stack(children: [
            Column(children: [
              ...
              _StatusBar(
                ...
                onToggleSeguir: () => setState(() {
                  ...
                  if (_seguirOperador && _seleccionadoId == null && ops.isNotEmpty) {
                    _seleccionadoId = ops.first.id;
                  }
            ...
            if (hasError)
              _Overlay(
                ...
                child  : const Text('Error cargando posiciones',
        ...
        child: Column(
          ...
          children: [
            ...
            Row(children: [
              ...
              Expanded(
                child: Text(item['camion'] ?? 'Sin camión',
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                    style: const TextStyle(
                        color     : _C.textSub,
...
class _TruckMarker extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        _PulseTruckIcon(
            color: color, bearing: bearing, enMovimiento: enMovimiento),
    ...
    return AnimatedBuilder(
      ...
      builder: (_, __) {
        ...
        return Stack(alignment: Alignment.center, children: [
          // Halo
          Container(
            width : 38 * pulse, height: 38 * pulse,
            decoration: BoxDecoration(
              shape: BoxShape.circle,
...
class _OperadorActivo {
  ...
  const _OperadorActivo({
    ...
    required this.posicion, required this.color,
  });
}
